...
function copy(element: JQuery<Element>, then: (success: boolean) => void) {
    element.select();
...
function updateProfileButton() {
    const name = Profile.current.hasName ? `${Profile.current.name} (${Profile.current.completionDescription})` : `${Profile.current.completionDescription} complete`;
...
function updateProfileDialog() {
    ...
    Profile.each(function(i, profile) {
        if (i !== Profile.currentIndex) {
            ...
            profileRows += `<button class="profileBecomeButton" type="button" value="${i}">Become ${name} (${profile.completionDescription})</button><br/>`;
        }
    });
    ...
    $("#profileSavedLink").onPushExactly(() => $("#profileSavedLink").select());
    $("#profileCopySavedLinkButton").button({
        icons: { primary: "ui-icon-link" }
    }).on("click", function() {
        copy($("#profileSavedLink"), function(success: boolean) {
            if (success) {
            ...
            if (isMobile()) { document.getSelection()?.removeAllRanges(); }
        });
    });
    ...
    if (Profile.recentlyDeleted !== null) {
        $("#profileReviveLink").onPushExactly(() => Profile.reviveRecentlyDeleted());
    }
    ...
    $("#profileDialog").dialog({
        ...
        beforeClose: () => document.getSelection()?.removeAllRanges()
    });
}
...
function initializeHelpButton() {
    ...
    $("#helpButton").onPushExactly(() => $("#helpDialog").dialog("open"));
}
